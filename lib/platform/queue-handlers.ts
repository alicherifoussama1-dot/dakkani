...
export function initPlatformRuntime(): void {
  ...
  // ── push.order: RETRY path for the merchant's new-order alert.
  //
  // Delivery itself happens inline in app/api/orders/route.ts. It has to:
  // this queue is drained by a cron that runs once a day at 03:00, so an
  // order alert that waited for the queue reached the merchant up to 24
  // hours late — indistinguishable from the feature not working. The order
  // route enqueues this job only when its inline send failed.
  //
  // Both paths call the same notifyNewOrder() so they cannot drift apart.
  registerHandler('push.order', async (job: Job) => {
    ...
    const res = await notifyNewOrder(orderId, storeId)

    // Surface a real failure so the queue retries with backoff. No registered
    // devices, or an order deleted before the retry ran, are not failures.
    if (!res.ok && res.reason === 'send-failed') {
      throw new Error(`push.order: all sends failed — ${res.error}`)
    }
  })
  ...
  // push.order is deliberately NOT subscribed — the order route delivers it
  // inline and enqueues the job only as a retry. Subscribing here would both
  // double-notify and put first delivery back on the once-a-day cron.
