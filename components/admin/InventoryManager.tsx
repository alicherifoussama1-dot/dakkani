      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative">
          ...
          <input
            ...
            className="bg-gray-800 border border-gray-700 rounded-lg pr-9 pl-3 py-1.5 text-sm text-gray-300 w-52 focus:ring-1 focus:ring-dakkani-500 outline-none"
          />
        </div>
        <div className="flex gap-1.5">
          {[['all','الكل'],['low','مخزون منخفض'],['out','نفد']] .map(([v,l]) => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg border transition ${filter===v ? 'bg-dakkani-500/20 text-dakkani-400 border-dakkani-500/30' : 'bg-gray-800 text-gray-500 border-gray-700'}`}
            >
              {l}
            </button>
          ))}
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm space-y-4" dir="rtl">
            ...
            <div className="flex gap-3">
              <button
                onClick={doTransfer}
                disabled={saving}
                className="flex-1 bg-dakkani-500 hover:bg-dakkani-600 text-white font-bold py-2 rounded-xl text-sm transition disabled:opacity-50"
              >
