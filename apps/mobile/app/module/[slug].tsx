...
import {
  Card, EmptyState, Loading, ErrorState, ListSkeleton,
    ...
    <>
      ...
      {providers.isError ? (
        ...
        <EmptyState
          ...
          sub="اربط شركة التوصيل من لوحة التحكم على الموقع لتظهر هنا."
        />
      ) : rows.map((p, i) => (
        <Card key={p.id} >
  ...
  if (!offices.length) {
    return (
      <EmptyState
        ...
        sub="تُضاف مكاتب الاستلام من لوحة التحكم على الموقع، أو تُجلب تلقائياً من شركة التوصيل."
      />
    )
  }
  ...
  return (
    <>
      ...
      {Object.keys(PLAN_LIMITS).map((p, i) => {
        ...
        return (
          <Card key={p} style={current ? styles.currentPlan : undefined}>
            <View style={styles.rowBetween}>
...
function UnknownModule({ name }: { name: string }) {
  ...
  return (
    <EmptyState
      ...
      sub={`تعذّر فتح «${name}». قد يكون الرابط قديماً.`}
    />
  )
}
...
const styles = StyleSheet.create({
  ...
  hint: { fontSize: 11, fontFamily: fontFamily.semibold, color: color.ink3, lineHeight: 18, marginTop: 10 },
  divider: { height: 1, backgroundColor: color.border, marginVertical: 12 },
  zoneRow: { flexDirection: 'row', alignItems: 'center', gap: 9, paddingVertical: 7 },
  zoneName: { fontSize: 13, fontFamily: fontFamily.bold, color: color.ink },
  zoneParent: { fontSize: 11, fontFamily: fontFamily.semibold, color: color.ink3, marginStart: 'auto' },
  rowBetween: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  planName: { fontSize: 15, fontFamily: fontFamily.bold, color: color.ink },
  ...
  badgeText: { fontSize: 10.5, fontFamily: fontFamily.bold, color: color.br700 },
  ...
  priceVal: { fontSize: 12.5, fontFamily: fontFamily.bold, color: color.ink, fontVariant: ['tabular-nums'] },
  officeRow: { flexDirection: 'row', alignItems: 'center', gap: 9, paddingVertical: 8 },
  usageVal: { fontSize: 13, fontFamily: fontFamily.bold, color: color.ink, fontVariant: ['tabular-nums'] },
  track: { height: 7, borderRadius: 4, backgroundColor: color.sunken, marginTop: 7, overflow: 'hidden' },
  trackFill: { height: '100%', borderRadius: 4 },
  heroLabel: { ...text('xs', 'medium'), color: color.ink2 },
  heroValue: { ...text('3xl', 'bold'), color: color.ink, marginVertical: 6 },
  ...
  kvV: { fontSize: 13.5, fontFamily: fontFamily.bold, color: color.ink, flexShrink: 1 },
})
