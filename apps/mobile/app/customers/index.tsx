...
export default function Customers() {
  ...
  return (
    <View style={styles.root}>
      <TopBar title="العملاء" onBack={() => router.back()} />
      ...
      <View style={styles.searchWrap}>
        ...
        {search ? (
          <Pressable onPress={() => setSearch('')} hitSlop={8} accessibilityRole="button" accessibilityLabel="مسح البحث"><IconClose size={16} color={color.ink3} /></Pressable>
        ) : null}
      </View>
        ...
        <FlashList<CustomerRow>
          ...
          ListEmptyComponent={
            <EmptyState icon={<IconUsers size={44} color={color.ink3} />}
              title="لا يوجد عملاء" sub={debounced ? 'لا نتائج لبحثك.' : 'سيظهر عملاؤك هنا بعد أول طلب.'} />
          }
...
const styles = StyleSheet.create({
  ...
  avatarText: { fontSize: 16, fontFamily: fontFamily.bold, color: color.br700 },
  name: { fontSize: 13.5, fontFamily: fontFamily.bold, color: color.ink, flexShrink: 1 },
  spend: { fontSize: 13.5, fontFamily: fontFamily.bold, color: color.br700, fontVariant: ['tabular-nums'] },
  phone: { fontSize: font.label, fontFamily: fontFamily.bold, color: color.ink3, marginTop: 3, writingDirection: 'ltr' },
  meta: { fontSize: font.label, fontFamily: fontFamily.bold, color: color.ink3, marginTop: 4, flexShrink: 1 },
})
