...
import { Card, Chips, ListSkeleton, EmptyState, ErrorState, StatusPill } from './ui'
      ...
      >
        {q.isError ? (
          ...
          <EmptyState icon={<IconOrders size={44} color={color.ink3} />}
            title="لا شيء في هذه القائمة" sub="كل الطلبات في هذه المرحلة تمّت معالجتها." />
        ) : (
          rows.map((o, i) => (
            <Card key={o.id} >
              <Pressable onPress={() => router.push(`/orders/${o.id}`)} accessibilityRole="button" accessibilityLabel={`فتح الطلب ${o.order_number ?? ''}`}>
              ...
              <View style={styles.actions}>
                <Pressable style={[styles.act, styles.actCall]} onPress={() => call(o)} accessibilityRole="button" accessibilityLabel="اتصال">
                  <IconPhone size={15} color={color.white} />
                  <Text style={styles.actTextW}>اتصال</Text>
                </Pressable>
                <Pressable style={[styles.act, styles.actWa]} onPress={() => wa(o)} accessibilityRole="button" accessibilityLabel="واتساب">
                  <IconWhatsApp size={15} color={color.br700} />
                  <Text style={styles.actText}>واتساب</Text>
                </Pressable>
                {o.status !== 'confirmed' && (
                  <Pressable style={[styles.act, styles.actOk]} accessibilityRole="button" accessibilityLabel="تأكيد"
...
const styles = StyleSheet.create({
  ...
  statValue: { fontSize: 17, fontFamily: fontFamily.bold, fontVariant: ['tabular-nums'] },
  statLabel: { fontSize: 9.5, fontFamily: fontFamily.bold, color: color.ink3, marginTop: 3 },
  ...
  phone: { fontSize: 12.5, fontFamily: fontFamily.bold, color: color.ink2, marginTop: 4, writingDirection: 'ltr' },
  total: { fontSize: 14, fontFamily: fontFamily.bold, color: color.br700, fontVariant: ['tabular-nums'] },
  ...
  actWa: { backgroundColor: color.white, borderWidth: 1, borderColor: color.border },
  actOk: { backgroundColor: color.br50 },
  actNo: { backgroundColor: color.rose50, flex: 0.5 },
  actText: { fontSize: 11.5, fontFamily: fontFamily.bold, color: color.ink },
  actTextW: { fontSize: 11.5, fontFamily: fontFamily.bold, color: color.white },
})
