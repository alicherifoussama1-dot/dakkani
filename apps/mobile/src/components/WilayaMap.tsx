...
import { color, radius, font, motion, fmtDZD, fmtNum , fontFamily } from '../theme/tokens'
...
function Tile({
  index, size, start, top, fill, highlight, selected, onPress, label,
}: {
  ...
  React.useEffect(() => {
    const d = index * 8
    o.value = withDelay(d, withTiming(1, { duration: 460, easing: Easing.bezier(...motion.standard) }))
    s.value = withDelay(d, withTiming(1, { duration: 460, easing: Easing.bezier(...motion.standard) }))
  }, [index, o, s])
...
const styles = StyleSheet.create({
  ...
  legendText: { fontSize: 10.5, fontFamily: fontFamily.bold, color: color.ink3 },
  scale: { flex: 1, height: 8, borderRadius: 5, overflow: 'hidden', flexDirection: 'row' },
  note: { fontSize: 10.5, fontFamily: fontFamily.bold, color: color.ink3, textAlign: 'center', paddingBottom: 4 },
  detail: {
    marginTop: 10, padding: 14, borderRadius: radius.md,
    backgroundColor: color.br50, borderWidth: 1, borderColor: color.br100,
  },
  detailAr: { fontSize: 16, fontFamily: fontFamily.bold, color: color.ink },
  detailFr: { fontSize: 12, fontFamily: fontFamily.semibold, color: color.ink3, marginTop: 2 },
  detailRow: { flexDirection: 'row', gap: 14, marginTop: 8 },
  detailStat: { fontSize: 13, fontFamily: fontFamily.bold, color: color.br700 },
})
