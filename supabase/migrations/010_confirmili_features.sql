-- ============================================================
-- Migration 010 — Confirmili Complete Feature Set (IDEMPOTENT)
-- Safe to run multiple times. Run AFTER migrations 001-009.
-- ============================================================
...
DROP POLICY IF EXISTS "tenant_isolation" ON confirmili_notifications;
DROP POLICY IF EXISTS "tenant_isolation" ON confirmili_delivery_companies;
DROP POLICY IF EXISTS "tenant_isolation" ON confirmili_wilaya_company_map;
DROP POLICY IF EXISTS "tenant_isolation" ON confirmili_price_lists;
DROP POLICY IF EXISTS "tenant_isolation" ON confirmili_price_list_wilayas;
DROP POLICY IF EXISTS "tenant_isolation" ON confirmili_team;
DROP POLICY IF EXISTS "tenant_isolation" ON confirmili_send_reports;
...
CREATE POLICY "tenant_isolation" ON confirmili_price_list_wilayas USING (
  price_list_id IN (
    SELECT pl.id
    FROM confirmili_price_lists pl
...
-- ── Realtime enable (guarded — won't error if already added) ──
DO $$
BEGIN
  BEGIN
